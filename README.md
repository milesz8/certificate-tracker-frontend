...
* [`src/cljs/certificate_tracker_frontend/`](src/cljs/certificate_tracker_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/certificate_tracker_frontend/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/certificate_tracker_frontend/`](test/cljs/certificate_tracker_frontend/): test files (ClojureScript,
...
7. Clone this repo and open a terminal in the `certificate-tracker-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/certificate_tracker_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns certificate-tracker-frontend.example
  (:require [certificate-tracker-frontend.config :as config])
